...
export default function VendorDashboardPage() {
    ...
    useEffect(() => {
        // Generate data on the client side to avoid hydration errors with Math.random()
        const generatedData = [
        ...
        setTotalRevenue(generatedData.reduce((acc, item) => acc + item.total, 0));
    }, []);
        ...
        <div className="flex flex-col gap-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <Card>
                    ...
                    <CardContent>
                        {isLoading ? <Skeleton className="h-8 w-1/2" /> : <div className="text-2xl font-bold">{totalRevenue.toLocaleString('fr-FR')} FCFA</div>}
                        {isLoading ? <Skeleton className="h-4 w-full mt-1" /> : <p className="text-xs text-muted-foreground">+20.1% depuis le mois dernier</p>}
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <CardContent>
                         {isLoading ? <Skeleton className="h-8 w-1/4" /> : <div className="text-2xl font-bold">+120</div>}
                         {isLoading ? <Skeleton className="h-4 w-full mt-1" /> : <p className="text-xs text-muted-foreground">+15% depuis le mois dernier</p>}
                    </CardContent>
                </Card>
                 <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                     ...
                     <CardContent>
                        {isLoading ? <Skeleton className="h-8 w-1/4" /> : <div className="text-2xl font-bold">8</div>}
                        {isLoading ? <Skeleton className="h-4 w-full mt-1" /> : <p className="text-xs text-muted-foreground">2 produits en rupture de stock</p>}
                    </CardContent>
                </Card>
            </div>
            <Card>
