...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ error: "Une erreur est survenue lors de l'initiation du paiement.", details: error.response?.data }, { status: 500 });
  }
}
...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ error: "Une erreur est survenue lors de la vérification du statut.", details: error.response?.data }, { status: 500 });
  }
}
